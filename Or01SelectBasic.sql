...
*/
-- 사원테이블에 저장된 모든 레코드르 ㄹ대상으로 전체 컬럼을 조회.
...
*/
select employee_idm, first_name, last_name, email, department_id
...
--number(숫자) 타입의 커럼끼리는 산술연산을 할 수 있다.
...
--방법2 : in을 사용하면 컬럼명을 한번만 기술하면 되므로 편하다. 이 방법을 써라.@!@!@!
...
--아래 2가지의 방법이 동일하므로 편한거 사용 2번이 더 편해보임
...
--distinct를 사용하면 중복이 제거가 됨 select뒤에 사용.
...
이 경우 첫번째 컬럼으로 정렬된 상태에서 주번째 컬럼이 정렬된다. */

/*
시나리오] 사원정보 테이블에서 급여가 낮은 순서대로 높은 순서로 인출되도록
정렬하여 인출하시오.*/
