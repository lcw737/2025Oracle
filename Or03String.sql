...
형식] lpad('문자열', '전체자리수', '채울문자')
    => 전체 자리수에서 문자열의 길이만큼을 제외한 나머지 와(우)측
    부분을 주어진 문자로 채워준다.
*/
...
substr() : 문자열에서 시작인덱스부터 길이만큼 잘라서 문자열을 출력
    ...
    참고2] '길이'에 해당하는 인수가 없으면 문자열의 끝까지 의미한다.
    참고3] 시작인덱스가 음수면 우측끝부터 좌로 인덱스를 적용한다.
*/
select substr('good morning john', 8, 4) from dual;--rnin
select substr('good morning john', 8) from dual;--r부터 끝까지 인출
...
*을 출력하면 첫글자를 제외한 남은 길이만큼을 마스킹 처리할 수 있다. */
select 
    first_name,
