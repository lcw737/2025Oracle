...
/*
sydate : 현재날짜와 시간을 초 단위로 반환해준다. 주로 게시판이나 회원가입시
    날짜를입력하기 위해 사용된다. 오라클의 기본 서식이 xx/xx/xx이므로 출력시
...
/*
날짜포맷 : 오라클은 대소문자를 구분하지 않으므로, 서식문자 역시 구번없이
    사용할 수 있다. 따라서 MM과mm은 동일한 결과를 인출한다.
*/
...
select to_char(sysdate, '오늘은 yyyy년dd월mm일 입니다.') from dual;
/*
-(하이픈), /(슬러쉬)와 같은 특수기호 외에는 서식으로 인식하지 못하므로
이를 제외한 나머지 문자열은 "으로 묶어줘야한다. 서식문자를 감싸는게 아니므로
작성제 주의해야한다. */
select to_char(sysdate, '"오늘은 "yyyy"년"dd"월"mm"일" 입니다."') from dual;
...
*/
--문자열 사이에 콤마가 포함되어 있으므로 숫자로 변환할 수 없어 에러가 발생됨 
...
처리해야한다. */
select to_date('02-04-2025')+1 from dual; --에러발생 
...
select 
    ...
    90,	'Executive',
    100, 'Finance', /*조건end*/
    '부서명확인안됨' /*defalut값으로 조건에 적용되지 않는 나머지 */
    )as dep_name
from employees;
