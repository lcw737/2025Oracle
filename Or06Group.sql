...
*/
--사원테이블에서 담당업부 인출. 별도의 조건이 없으므로 107개가 인출됨.
...
인출된 결과르 80번 부서라는걸 알 수 있다. */
select * from departments where upper(department_name) ='SALES';
--3. 80번 부서에서 근무하느 ㄴ사원들의 평균급여를 구할 수 있다.
select
...
이와같은경우에는 뒤에서 학습할 '서브쿼리'를 사용해야한다. */
...
추가하면 에러가 발생한다. 이 경우에는 having절에 조건을 추가해야한다.
...
--복습 order by는 기본적으로 오름차순 asc는 생략가능 내림차순은 desc를 붙여야함.
