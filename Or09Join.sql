...
검색할때 사용한다.
-일반적으로 기본키(Primary key)와 외래키(Foreign key)를 사용하는 join하는
경우가 대부분이다.
...
레코드가 인출된다. 즉 onner join은 조인한 테이블 양쪽에 만족되는
레코드를 대상으로 인출하게 된다. */
...
*/
--1.지역 테이블을 통해 seattle이 위치한 레코드의 일변번호를 확인 
...
outer join을 사용할 떄는 반드시 기준이 되는 테이블을 결정하고 쿼리문을
작성해야한다.
...
-위의 경우 왼쪽 테이블이 변경된다.
-기준디 되는 테이블을 변경하고 싶다면 테이블의 위치를 옮겨준다.
    (+)를 옮기지 않는다.
*/
/*
시나리오] 전체직원의 사원번호, 이름, 부서아이디, 부서명, 도시명을
...
/*
3] self join(셀프조인)
셀프조인은 하나의 테이블에있는 컬럼끼리 조인하는 경우에 사용한다.
즉 자신의 테이블과 조인을 맺는것이다.
셀프조인에서는 별칭이 테이블을 구분하는 구반자의 역할을 하므로 매우
중요하다.
형식 ] 
    select
        별빛1.컬럼, 별칭2.컬럼 ....
...
/*
    ...
    컬럼인것을 전재로 작성되기 떄문에 굳이 별칭을 사용할 필요가 없기때문이다.
...
*/
--select
--     employee, first_name, last_name, email, department_name, 
--     salary, job_title, COUNTRY_NAME, hire_date
--from locations
...
--문제에서 주어진 지역정보를 지역 일련번호로 확인 (결과 : 지역번호 1500)
...
select
    ...
    to_char(salary,'$9,000'), country_id, country_name
from employees
